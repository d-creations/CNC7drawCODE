...
export class GeometricHorizontalShape extends BaseShape {
    ...
    getRenderData() {
        ...
        // Short horizontal blue bar centered at the midpoint
        const barLength = 10;
        ...
        return [{
            primitive: 'line',
            worldStartX: midX - worldBarLength,
            worldStartY: midY,
            worldEndX: midX + worldBarLength,
            worldEndY: midY,
            color: this.color,
            lineWidth: 2
        }];
    ...
    buildProperties(editor) {
        ...
        divArea.innerHTML = `<h4 style="margin:0 0 5px 0">Horizontal Constraint</h4><p>Forces the selected points to share the same Y axis.</p>`;
        ...
        rmBtn.onclick = () => {
             ...
             if (this.constraintId) {
                 ...
                 for (let [cId, cDef] of editor.drawBoard.constraintSystem.constraints) {
                     if (cDef.geometryId === this.constraintId || (cDef.targets.includes(this.p1.constraintId) && cDef.targets.includes(this.p2.constraintId) && cDef.type === "Horizontal")) {
